// CONTEXT TAB – T.E.R.A. v13 / v18-fix / v18-cc / v18-lola
...
// v18-lola: LoLa (Localized Languages) integration.
//           TERA builds and enriches the full context first, then LoLa
//           processes the result for language handling. This ordering
//           ensures LoLa's language directives and truncation guard apply
//           to all TERA-injected content (anchors, card narratives, hints).
//           LoLa must be pasted into the Library tab below the TERA engine.
//           If LoLa is not installed, the three lola() calls below are
//           no-ops — set USE_LOLA = false to skip them entirely.

InnerSelf("context");

// ── LoLa toggle ───────────────────────────────────────────────────────
// Set to false if you are not using Localized Languages.
const USE_LOLA = true;
...
// ── LoLa wrapper ─────────────────────────────────────────────────────
// Safely calls LocalizedLanguages("context", text) if LoLa is installed
// and USE_LOLA is true. Returns text unchanged if LoLa is absent.
function _applyLola(text) {
  if (!USE_LOLA) return text;
  if (typeof LocalizedLanguages !== "function") return text;
  return LocalizedLanguages("context", text);
}
...
const modifier = (text) => {
  ...
  // TERA enriches first, then LoLa processes the complete result.
  ...
  if (enrichedWithEvent.length - text.length <= TOTAL_CONTEXT_CAP) {
    return { text: _applyLola(enrichedWithEvent), stop: false };
  }
  ...
  if (enrichedNoEvent.length - text.length <= TOTAL_CONTEXT_CAP) {
    return { text: _applyLola(enrichedNoEvent), stop: false };
  }

  // ── Tier 3: minimal hint only (story history very long) ─────────────
  return { text: _applyLola(hint + text), stop: false };
};
