...
function _loadConfig() {
  ...
  if (!configCard || !configCard.entry) return;
  for (const line of configCard.entry.split("\n")) {
  ...
  _applyScenarioMode(configCard.entry);
}
...
function _teraEnsureRegistry() {
  ...
  for (const card of storyCards) {
    ...
    seen.add(cardId);
    const hash = _hash(k + "\0" + (card.entry || ""));
    const cached = di[cardId];
    if (cached && cached.h === hash) {
      const { narrative, effectsText } = _splitEntry(card.entry || "");
      CARD_REGISTRY[cardId] = Object.assign({}, cached.meta, { narrative, effectsText });
    } else {
      const parsed = _parseCardKey(k, card.entry || "", card.description || "");
      if (parsed) {
        const gates = [];
...
function _runBootstrap() {
  ...
  t._setupDone = true;
  if (!setupCard || !setupCard.entry) return;

  const log = [];
  for (const rawLine of setupCard.entry.split("\n")) {
...
function _teraValidate(cardId) {
  ...
  const fxMatch = (sc.entry || "").match(/\[FX\]([\s\S]*?)\[\/FX\]/);
  if (fxMatch) {
